...
class ModelCFG(nn.Module):
    '''
    This code is adapted from Henry Schreiner's core.py and Will Tepe's model config files, but 
    with a few key adjustments. For one, this file attempts to generalize more parameters 
    (e.g. skip connections, batch normalization). For another point, this file attempts to 
    modernize some of the setup to account for the current use of mlflow and its model weights 
    saving structure.
    '''
    LEAKYNESS = 0.01
    ...
    # parameters can be overriden in the class declaration (see __init__())
    leakyness = 0.01
    kernel_size = None       # must be overriden
    channel_size = None      # must be overriden
    skip_connection = None   # must be overriden
    fc = False               # off by default
    dropout = True           # on by default
    drop_rate = .15          # set drop rate
    inputs = 1               # number of inputs (1 for z, 3 for x and y too)
    ...
    def __init__(self, **kwargs):
        ...
        for i in range(nlayers):
            if skip_connection is None:
                ...
                assert len(skip_connection) == nlayers, "You need to include each layer's skip connection(s)"
