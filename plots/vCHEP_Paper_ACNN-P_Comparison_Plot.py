...
for data, color, mi, group in zip(data, colors, markers, groups):
    x, y = data
    ax.plot(x,y,c=color,marker=mi,fillstyle='none',linestyle='none',markersize=13,label=group)
...
for data, color, mi, group in zip(data, colors, markers, groups):
    x, y = data
    ax.plot(x,y,c=color,marker=mi,linestyle='none',markersize=13,label=group)
    ax.legend(loc='upper left', fontsize=23)
...
plt.text(0.97,0.07,'Toy MC Simulation',horizontalalignment='right',
